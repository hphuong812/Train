...
namespace MessagePack.Unity.Editor
{
    internal class MessagePackWindow : EditorWindow
    {
        ...
        async void OnGUI()
        {
            ...
            if (GUILayout.Button("Generate"))
            {
                ...
                try
                {
                    Debug.Log($"Run: {ProcessHelper.dotnetPath} mpc {commnadLineArguments}");
                    var log = await ProcessHelper.InvokeProcessStartAsync(ProcessHelper.dotnetPath, "mpc " +commnadLineArguments);
                    UnityEngine.Debug.Log(log);
                }
                finally
                {
    ...
    internal static class ProcessHelper
    {
        ...
        public const string dotnetPath = "/usr/local/share/dotnet/dotnet";

        public static async Task<bool> IsInstalledMpc()
        {
            var list = await InvokeProcessStartAsync(dotnetPath, "tool list -g");
        ...
        {
            return await InvokeProcessStartAsync(dotnetPath, "tool install --global " + InstallName);
        }
        ...
        public static async Task<(bool found, string version)> FindDotnetAsync()
        {
            try
            {
                var version = await InvokeProcessStartAsync(dotnetPath, "--version");
                return (true, version);
            }
            catch(Exception e)
            {
