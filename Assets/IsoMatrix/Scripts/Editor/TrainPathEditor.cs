...
namespace IsoMatrix.Scripts.Editor
{
    [CustomEditor(typeof(TrainPath))]
    sealed class TrainPathEditor : BaseEditor<TrainPath>
    {
        public static string kPreferTangentSelectionKey = "TrainPathEditor.PreferTangentSelection";
        ...
        void DrawWaypointEditor(Rect rect, int index)
        {
            ...
            TrainPath.Waypoint def = new TrainPath.Waypoint();
            ...
            if (GUI.Button(r, buttonContent, style) && SceneView.lastActiveSceneView != null)
            {
                ...
                TrainPath.Waypoint wp = Target.m_Waypoints[index];
            ...
            if (GUI.Button(r, buttonContent, style))
            {
                ...
                var list = new List<TrainPath.Waypoint>(Target.m_Waypoints);
                list.RemoveAt(index);
        ...
        {
            TrainPath.Waypoint wp = Target.m_Waypoints[i];
        ...
        [DrawGizmo(GizmoType.Active | GizmoType.NotInSelectionHierarchy
             | GizmoType.InSelectionHierarchy | GizmoType.Pickable, typeof(TrainPath))]
        static void DrawGizmos(TrainPath path, GizmoType selectionType)
        {
        ...
        static int LocalSpaceSamplePath(TrainPathBase pathBase, int samplesPerSegment, Vector3[] positions, Quaternion[] rotations)
        {
            var path = pathBase as TrainPath;
        ...
        public delegate int BatchSamplerDelegate(TrainPathBase path, int samplesPerSegment, Vector3[] positions, Quaternion[] rotations);

        public static void DrawPathGizmo(TrainPathBase path, Color pathColor, bool isActive, BatchSamplerDelegate batchSampler = null)
        {
        ...
        static int DefaultBatchSampler(TrainPathBase path, int samplesPerSegment, Vector3[] positions, Quaternion[] rotations)
        {
            int numSamples = 0;
